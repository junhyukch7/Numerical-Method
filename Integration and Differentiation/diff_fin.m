function [derivative] = diff_fin(var,dim,accuracy,order,d_dim,type)
%% DERIVATIVE FINITE DIFFERENCE
% function [derivative] = fintediff(var,dim,accuracy,order,d_dim,type)
% input:
% var:          [xi;yi]'
% xi:           [xi-2 xi-1 xi xi+1 xi+2];
% yi:           [f(xi-2) f(xi-1) f(xi) f(xi+1) f(xi+2)]
% dim:          함수에서 변수의 개수를 의미
% accuracy:     정확도=h^n
% order:        n차 도함수
% d_dim:        h 간격을 의미함
% type:         'central' or 'forward' or 'backward'
% output : result(3,2)가 내가 구하고 자하는 값
switch nargin
    case 5;        type = 'central'; 
...
%  d2u/dx2 =  ( - A(1,1)*u(n,:) - A(1,2)*u(n-1) - A(1,3)*u(n-2) )/dx^2 % Fist order accurate backward difference
    for term_cnt = 1:accuracy+2 %% 여기 오류 였음
